...
export function CoupleThings() {
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.getProducts({ category: "Couple Things" }).then((res) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <section className="relative h-[50vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-rose-50 via-white to-pink-50">
        <div className="text-center px-4 max-w-4xl mx-auto">
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="text-sm font-semibold text-rose-600 uppercase tracking-wider mb-4"
          >
            Collection
          </motion.p>
          <motion.h1
          ...
          >
            Celebrate love together with our thoughtfully curated couple collections.
          </motion.p>
        </div>
      </section>
